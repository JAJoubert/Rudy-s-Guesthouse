...
from dailynumbers import numbers
...
for i in range(5):
    ...
    X = data[['num1', 'num2', 'num3', 'num4']].applymap(lambda x: x if x <= 36 else 0)
    y = data[f'num{i+1}']
    ...
    while next_num in prev_nums or next_num in last_nums:
        next_num = int(model.predict(X.tail(1))[0])
    ...
    for j in range(5):
        possible_num = next_num+j if next_num+j <= 36 else next_num+j-36
        if possible_num not in last_nums:
            print(possible_num)
